...
import addon.bevy_client as bevy_client

class CustomRenderEngine(bpy.types.RenderEngine):
    # These three members are used by blender to set up the
    # RenderEngine; define its internal name, visible name and capabilities.
    bl_idname = "FromBevyGame"
    bl_label = "From Bevy Game"
    bl_use_preview = True
    ...
    def view_draw(self, context, depsgraph):
        from addon.bevy_client import get_data
        ...
        try:
            pixels = gpu.types.Buffer('FLOAT', len(data), data)
            width = height = int(sqrt(len(frame) / 16))
